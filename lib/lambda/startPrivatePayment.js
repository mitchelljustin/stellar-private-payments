...
const convertPaymentSize = require('../helpers/convertPaymentSize')
...
module.exports.entry = (event, context, done) => {
    ...
    if (!payment.size instanceof String) {
        throw new Error("Payment size must be string")
    }
    let totalPaymentSize = convertPaymentSize.fromString(payment.size)
    let paymentSizes
    try {
    ...
    let privPmts = paymentSizes.map(paymentSize => {
        ...
        return {
            ...
            size: convertPaymentSize.toString(paymentSize),
        }
    })
    Async.map(
        privPmts,
